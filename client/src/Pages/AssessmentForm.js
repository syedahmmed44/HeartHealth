...
function ClinicalAssessmentForm() {
  // State to manage the form input and modal visibility
  const [testData, setTestData] = useState({
    testName: '',
    attributeName: '',
    unit: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestData = {
        ...
        attribute_name: testData.attributeName,
        unit: testData.unit,
      ...
      if (userEmail) {
        // Make a POST request to fetch LOINC code
        ...
        // Assuming the response contains the LOINC code and related info
        const loincData = response.data;

        // Update the prediction data for displaying in the modal
        setPredictionData({
          loincCode: loincData.loinc_code,
          description: loincData.description,
          additionalInfo: loincData.additional_info, // Adjust as per your backend response
        });
  ...
  return (
    <div className="clinical-assessment-form">
      ...
      <Navbar bg="dark" variant="dark" expand="lg">
        <Container>
          <Navbar.Brand href="#home">Clinical Assessment</Navbar.Brand>
      ...
      <h2 className="mt-4">Clinical Assessment Form</h2>
      ...
      {/* Modal to display the LOINC Code and related information */}
      <Modal show={showModal} onHide={handleCloseModal} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title className="text-primary">LOINC Code and Test Information</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div>
            <strong>Important!</strong> The following results are based on machine learning models and may not be fully accurate. For more reliable results, consult a specialist.
          </div>

          <div>
            <p><strong>Test Name:</strong> {testData.testName}</p>
            <p><strong>LOINC Code:</strong> {predictionData.loincCode}</p>
            <p><strong>Description:</strong> {predictionData.description}</p>
            <p><strong>Additional Info:</strong> {predictionData.additionalInfo}</p>
          </div>
        </Modal.Body>
        <Modal.Footer>
